  ...
  private
    ...
    def image_fields(form)
      form.fields_for image_fields_name, attachable, child_index: nested_index do |image_builder|
        render Images::FieldsComponent.new(image_builder, imageable: resource) do
          image_suggestions(form)
        end
      end
    end
    ...
    def image_suggestions(form)
      if resource.is_a?(Budget::Investment) && attachable.errors.any?
        render ImageSuggestions::SuggestButtonComponent.new(form)
      end
    end
