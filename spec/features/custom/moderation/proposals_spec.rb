...
feature 'Moderate proposals' do
  ...
  feature '/moderation/ screen' do
    ...
    feature 'moderate in bulk' do
      ...
      feature "When we review the proposal areas" do
        background do
          @proposal = create(:proposal)
          visit moderation_proposals_path
          within('.menu.simple') do
            click_link "Pending area review"
          end

          within("#proposal_#{@proposal.id}") do
            select "Municipal", from: "proposal_#{@proposal.id}_area"
          end

          expect(page).to_not have_css("proposal_#{@proposal.id}")
        end

        scenario 'Update proposal area' do
          click_button "Update areas"
          expect(@proposal.reload.area).to eq('municipal')
          expect(@proposal.reload).to be_area_revised
          expect(@proposal.reload).to_not be_hidden
        end
      end
