...
describe PersonApi do
  ...
  describe '#call' do
    let(:invalid_body) { {consultar_datos_response: {} } }
    let(:valid_body){ {consultar_datos_response: {datos_solicitante: {datos_nacimiento: {fecha: "19700101"}}}} }

    it "returns the response for the first valid variant" do
      allow(api).to receive(:get_response_body).with(1, "00123456", "Official name", "11111111Z").and_return(invalid_body)
      allow(api).to receive(:get_response_body).with(1, "123456", "Official name", "11111111Z").and_return(invalid_body)
      expect(api).to receive(:get_response_body).with(1, "0123456", "Official name", "11111111Z").and_return(valid_body)

      response = api.call(1, "123456", "Official name", "11111111Z")

      expect(response).to be_valid
      expect(response.date_of_birth).to eq(Date.new(1970,1,1))
    end

    it "returns the last failed response" do
      expect(api).to receive(:get_response_body).with(1, "00123456", "Official name", "11111111Z").and_return(invalid_body)
      expect(api).to receive(:get_response_body).with(1, "123456", "Official name", "11111111Z").and_return(invalid_body)
      expect(api).to receive(:get_response_body).with(1, "0123456", "Official name", "11111111Z").and_return(invalid_body)
      response = api.call(1, "123456", "Official name", "11111111Z")
